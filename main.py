...
train_text = (csds_datasets['train']['text'])
train_labels = (csds_datasets['train']['labels'])

test_text = (csds_datasets['eval']['text'])
test_labels = (csds_datasets['eval']['labels'])
print(len(train_text) + len(test_text))
train_labels = [str(x) for x in train_labels]
test_labels = [str(x) for x in test_labels]
print((test_labels))
class DataLoaderT5(Dataset):
    def __init__(self, type, max_len, tokenizer):
        if type == 'train':
            self.text, self.labels =train_text, train_labels
        elif type == 'test':
            self.text, self.labels = test_text, test_labels
...
class T5FineTuner(pl.LightningModule):
    ...
    def train_dataloader(self):
        train_dataset = DataLoaderT5("train", max_len=512, tokenizer=tokenizer)
...
test_dataset = DataLoaderT5("test", tokenizer=tokenizer, max_len=512)
...
args_dict = dict(
    ...
    output_dir="/home/jmurzaku/cogstates", # path to save the checkpoints
