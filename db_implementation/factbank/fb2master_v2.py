...
class FB2Master:
    ...
    def load_data(self):
        ...
        for row in sentences_sql_return:
            ...
            self.ma_cur.execute('INSERT INTO sentences (file, file_sentence_id, sentence)'
                                'VALUES (?, ?, ?);',
                                (row[self.FILE][1:-1], row[self.SENTENCE_ID], row[self.SENTENCE]))
            ...
            for current_nesting_level in range(0, 3):
                ...
                for rel_source_text in sources_sql_return:
                    ...
                    elif relevant_source is None:
                        global_source_token_id = 1
                    else:
                    ...
                    if current_nesting_level == 1:
                        ...
                        if parent_source_text is not None:
                    ...
                    # print('row[self.FILE]:', row[self.FILE])
                    # print('sentID:', row[self.SENTENCE_ID])
                    # print('relSourceText:', rel_source_text)
                    # print('eid_label_sql_return:', eid_label_sql_return)
