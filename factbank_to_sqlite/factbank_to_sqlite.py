...
##### CREATES THE CSDS TARGET AND SOURCE TABLES #####
#textReferent
  cur.execute('''CREATE TABLE IF NOT EXISTS textReferent
               (textPassages BLOB,
               name TEXT,
               type TEXT,
               subtype TEXT)''')
...
#textPassage
  cur.execute('''CREATE TABLE IF NOT EXISTS textPassage
               (referent BLOB,
               sentence TEXT,
               sentence_id INTEGER,
               document_id TEXT,
...
#csds 
  cur.execute('''CREATE TABLE IF NOT EXISTS csds
               (source BLOB,
               target BLOB,
               belief BLOB,
               sentiment BLOB)''')

##### INSERT CSDS DATA INTO CREATED SQLite TABLES #####
#textReferent
  # cur.execute("INSERT INTO textReferent VALUES (?, ?, ?, ?)", tuple(entry))
  # source (list) ["Natalie", "she", "her"]
  # target (points to textpassage), default value None
  # belief (default value is "")
  # sentiment (sentiment value is "")

#textPassage
  # cur.execute("INSERT INTO textPassage VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", tuple(entry))
  # referent (points to text referent)
  # sentence_id
  # document id 
  # head_start
  # head_end
  ...
  # span_end
  # span
  # isAuthor, bool default value is True

#csds
  # cur.execute("INSERT INTO csds VALUES (?, ?, ?, ?)", tuple(entry))
  # source (list)
  # target points to textpassage
  # belief 
  # sentiment
