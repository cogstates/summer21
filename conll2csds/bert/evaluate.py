...
from conll2csds.bert.bert_custom import BertEFP
from conll2csds.bert.dataloader import FactbankLoader
...
def correlation(logits, labels):
    r = pearsonr(logits.cpu().detach().numpy().flatten(), labels.cpu().detach().numpy())
...
def evaluate(model, loss, dataloader, device):
    ...
    with torch.no_grad():
        for input_ids, attention_mask, labels in tqdm(dataloader, desc="Evaluating"):
            ...
            logits_r = logits_r + logits.cpu().detach().numpy().flatten().tolist()
            labels_r = labels_r + labels.cpu().detach().numpy().tolist()
            count += 1
    return pearsonr(logits_r, labels_r)[0], mean_loss / count
...
if __name__ == "__main__":
    ...
    val_set = FactbankLoader(file_path="/home/jmurzaku/cogstates/conll2csds/factbank_v1/test.conll", max_len=args.maxlen_val, tokenizer=tokenizer)
